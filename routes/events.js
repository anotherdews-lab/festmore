// routes/events.js — WITH FREE LISTING OPTION + APPLY TO EVENT
...
router.post('/submit', async (req, res) => {
  ...
  if (plan === 'free') {
    db.prepare(`INSERT INTO events (title,slug,category,city,country,start_date,end_date,date_display,description,price_display,website,ticket_url,attendees,vendor_spots,address,tags,status,payment_status,source,featured) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'active','free','manual',0)`)
  ...
  const result = db.prepare(`INSERT INTO events (title,slug,category,city,country,start_date,end_date,date_display,description,price_display,website,ticket_url,attendees,vendor_spots,address,tags,status,payment_status,source,featured) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'pending','unpaid','manual',?)`)
    .run(title,slug,category,city,country,start_date,end_date||'',date_display||'',description||'',price_display||'Free',website||'',ticket_url||'',parseInt(attendees)||0,parseInt(vendor_spots)||0,address||'',tags||'[]',isPremium?1:0);
...
<div style="max-width:580px;margin:0 auto;text-align:center;padding:48px 32px;">
  ...
  <p style="color:var(--ink3);font-size:16px;line-height:1.75;margin-bottom:28px;">${plan === 'free' ? 'Your free listing is now live on Festmore. Upgrade to Standard or Premium anytime for featured placement, SEO boost and newsletter inclusion.' : 'Your event is now featured on Festmore and visible to thousands of visitors worldwide.'}</p>
...
<meta name="description" content="List your event on Festmore. Free, Standard (€79/yr) and Premium (€149/yr) plans."/>
...
<div class="container" style="padding:64px 0;max-width:1100px;">
  ...
  <div style="max-width:700px;margin:0 auto;">
    ...
    ${[['Can I upgrade from Free to Standard later?','Yes! You can upgrade your listing at any time from your dashboard.'],['How quickly will my event go live?','Free listings go live immediately. Standard and Premium listings go live as soon as payment is confirmed.'],['What countries can I list events for?','You can list events from any of our 26 covered countries worldwide.'],['Can I cancel my listing?','Standard and Premium listings are annual. You can request a refund within 14 days of purchase.'],['What payment methods do you accept?','We accept all major credit and debit cards through our secure Stripe payment system.'],['Is the free listing really free forever?','Yes — the free tier is free forever. We will never charge you without your consent.']].map(([q,a])=>`
...
.plan-card:hover{border-color:var(--flame);}
...
.plan-popular{position:absolute;top:-12px;left:50%;transform:translateX(-50%);background:var(--flame);color:#fff;font-size:10px;font-weight:800;padding:3px 12px;border-radius:99px;white-space:nowrap;}
...
${renderNavSimple(user)}
<div style="background:linear-gradient(135deg,#1a0a00,#3d1200);padding:64px 0;">
  <div class="container" style="max-width:860px;text-align:center;">
    <h1 style="font-family:'DM Serif Display',serif;font-size:clamp(28px,4vw,52px);font-weight:400;color:#fff;margin-bottom:14px;">Get Your Event in Front of Thousands</h1>
    <p style="color:rgba(255,255,255,.55);font-size:16px;">Free listing available. Upgrade for maximum visibility.</p>
  </div>
</div>
<div class="container" style="padding:48px 0;max-width:880px;">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:12px;">
    ${[{id:'free',label:'Free',price:'€0',period:'forever',color:'var(--sage)',desc:'Get started today'},{id:'standard',label:'Standard',price:'€79',period:'/year',color:'var(--flame)',desc:'Most popular',popular:true},{id:'premium',label:'Premium',price:'€149',period:'/year',color:'var(--gold)',desc:'Maximum exposure'}].map(p=>`
  ...
  ${[{id:'free',color:'rgba(74,124,89,.07)',border:'rgba(74,124,89,.2)',tc:'var(--sage)',title:'Free Plan:',desc:'✅ Basic listing · ✅ Visible in search · ❌ No featured · ❌ No newsletter · ❌ No SEO boost'},{id:'standard',color:'rgba(232,71,10,.05)',border:'rgba(232,71,10,.2)',tc:'var(--flame)',title:'Standard — €79/year:',desc:'✅ Full SEO · ✅ Featured · ✅ Newsletter · ✅ Analytics · ✅ Connect vendors · ✅ Priority support'},{id:'premium',color:'rgba(201,146,42,.05)',border:'rgba(201,146,42,.25)',tc:'var(--gold)',title:'Premium — €149/year:',desc:'⭐ Everything in Standard · ⭐ Top placement · ⭐ Homepage featured · ⭐ Social media · ⭐ Account manager'}].map(p=>`
  <div id="plan-info-${p.id}" class="plan-features ${plan===p.id?'visible':''}" style="background:${p.color};border:1px solid ${p.border};">
    <div style="font-weight:700;color:${p.tc};margin-bottom:6px;">${p.title}</div>
    <div style="font-size:13.5px;color:var(--ink2);">${p.desc}</div>
  </div>`).join('')}
  ...
  <form method="POST" action="/events/submit" id="submit-form">
    ...
    <div style="${SS}">
      <h2 style="font-family:'DM Serif Display',serif;font-size:22px;font-weight:400;margin-bottom:20px;">1. Event Details</h2>
    ...
    <div style="${SS}">
      <h2 style="font-family:'DM Serif Display',serif;font-size:22px;font-weight:400;margin-bottom:20px;">2. Your Contact Details</h2>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
    ...
    <div style="background:#fff;border:1px solid var(--border);border-radius:20px;padding:24px;margin-bottom:20px;">
      <label style="display:flex;align-items:flex-start;gap:12px;cursor:pointer;">
        <input type="checkbox" name="terms_agreed" value="1" required style="margin-top:2px;accent-color:var(--flame);width:18px;height:18px;flex-shrink:0;"/>
        <span style="font-size:13px;color:var(--ink2);line-height:1.6;">I agree to the Terms and Conditions including the no refund policy. I confirm my listing complies with all applicable laws.</span>
      </label>
    </div>
    ...
    <div style="background:var(--ink);border-radius:20px;padding:36px;margin-bottom:24px;">
      <div style="display:grid;grid-template-columns:1fr auto;gap:24px;align-items:center;">
        <div>
          ...
          <p style="color:rgba(255,255,255,.5);font-size:14px;line-height:1.6;margin-bottom:0;" id="plan-desc-text">${plan==='free'?'Free listing — goes live within 24 hours.':plan==='premium'?'Premium — top placement, homepage feature, dedicated newsletter.':'Standard — full SEO, newsletter, featured placement.'}</p>
        </div>
        <div style="text-align:center;flex-shrink:0;">
          ...
          <button type="submit" id="submit-btn" style="background:${plan==='free'?'#4a7c59':'#e8470a'};color:#fff;border:none;padding:16px 36px;border-radius:12px;font-size:16px;font-weight:700;cursor:pointer;font-family:inherit;">
...
function selectPlan(plan) {
  ['free','standard','premium'].forEach(p => {
    document.getElementById('plan-'+p).className = 'plan-card' + (p===plan?' active-'+p:'');
  ...
  const descs = {free:'Free listing — goes live within 24 hours.',standard:'Standard — full SEO, newsletter, featured placement.',premium:'Premium — top placement, homepage feature, dedicated newsletter.'};
...
// ─────────────────────────────────────
// EVENT DETAIL — COMPLETE WITH APPLY BUTTON
// ─────────────────────────────────────
function renderEventDetail(e, related, user) {
  ...
  const applySection = user
    ? `<div id="apply-section">
        <button onclick="toggleApply()" class="btn btn-primary" style="width:100%;margin-bottom:8px;">Apply to This Event →</button>
...
<div class="container" style="padding:40px 0;display:grid;grid-template-columns:1fr 340px;gap:40px;align-items:start;">
  ...
  <aside>
    ...
    <!-- UPGRADE CTA (only for free listings) -->
    ${isFree ? `
...
function shareEvent() {
  if (navigator.share) {
    navigator.share({ title: '${e.title.replace(/'/g,"\\'")}', url: window.location.href });
  } else {
  ...
  <a href="/events/${e.slug}">
    <div class="event-img">
      ...
      <div class="event-badges">
        ...
        ${isFreeList ? '<span class="badge" style="background:rgba(0,0,0,.4);color:#fff;">🔓 Unverified</span>' : '<span class="badge" style="background:#4a7c59;color:#fff;">✅ Verified</span>'}
      </div>
    </div>
...
function renderNavSimple(user) {
  ...
  <div class="nav-mobile">
    ...
    <a href="/events/submit">+ List Event</a>
