// routes/dashboard.js — FIXED v2
// KEY FIX: Vendors looked up by EMAIL not user_id
// This ensures vendors always see their profile after login
...
router.get('/', requireLogin, (req, res) => {
  ...
  // Get events by user_id OR email
  const myEvents = db.prepare("SELECT * FROM events WHERE user_id=? ORDER BY created_at DESC").all(user.id);

  // KEY FIX: Get vendors by EMAIL (not user_id) so vendors always see their profile
  const myVendors = db.prepare("SELECT * FROM vendors WHERE email=? AND status='active' ORDER BY created_at DESC").all(user.email);

  // Also link any unlinked vendors to this user
  try {
    db.prepare("UPDATE vendors SET user_id=? WHERE email=? AND (user_id IS NULL OR user_id=0)").run(user.id, user.email);
  } catch(e) {}

  const totalViews = myEvents.reduce((s, e) => s + (e.views || 0), 0);

  // If vendor role and has vendor profiles — show vendor-focused dashboard
...
<div class="container" style="padding:44px 0 80px;">
  ...
  <div style="margin-bottom:36px;display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:16px;">
    ...
    <div style="display:flex;gap:10px;flex-wrap:wrap;">
      ${isVendor ? '' : '<a href="/events/submit" class="btn btn-primary">+ List New Event</a>'}
  ...
  <div class="dashboard-stat-grid" style="margin-bottom:40px;">
    ${isVendor ? `
    <div class="dashboard-stat"><div class="dash-stat-n" style="color:var(--flame);">${myVendors.length}</div><div class="dash-stat-l">Vendor Profile${myVendors.length!==1?'s':''}</div></div>
  ...
  <!-- VENDOR PROFILES — shown prominently for vendors -->
  ${myVendors.length > 0 ? `
  <div style="background:#fff;border:1px solid var(--border);border-radius:20px;padding:28px 32px;margin-bottom:28px;">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:18px;">
      ${myVendors.map(v => {
        ...
        return `
      <div style="border:2px solid ${v.verified?'#86efac':'var(--border)'};border-radius:16px;overflow:hidden;">
  ...
  <div style="background:#fff;border:2px dashed var(--border2);border-radius:20px;padding:48px;text-align:center;margin-bottom:28px;">
    ...
    <p style="color:var(--ink3);margin-bottom:24px;max-width:400px;margin-left:auto;margin-right:auto;">If you already paid, please contact us at <a href="mailto:contact@festmore.com" style="color:var(--flame);">contact@festmore.com</a> and we will fix it immediately.</p>
    <a href="/vendors/register" class="btn btn-primary btn-lg">Create Vendor Profile — €49/yr →</a>
  </div>` : ''}`}

  <!-- EVENTS SECTION — only show for non-vendors or organisers -->
  ${!isVendor || myEvents.length > 0 ? `
  <div style="background:#fff;border:1px solid var(--border);border-radius:20px;padding:28px 32px;margin-bottom:28px;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:22px;">
          ...
          <tr style="border-bottom:1px solid var(--border);" onmouseover="this.style.background='var(--ivory)'" onmouseout="this.style.background=''">
            ...
            <td style="padding:14px 12px;">
              <div style="display:flex;gap:6px;">
                ...
                <a href="/dashboard/event/${e.id}" class="btn btn-primary btn-sm">Edit</a>
              </div>
            </td>
          </tr>`).join('')}
        </tbody>
  ...
  <div style="background:linear-gradient(135deg,#0d1f15,#1a3d28);border-radius:20px;padding:40px;text-align:center;">
    ...
    <p style="color:rgba(255,255,255,.55);font-size:15px;margin-bottom:24px;">Browse 174+ festivals and markets looking for vendors like you.</p>
...
// ─────────────────────────────────────
// VENDOR EDIT — redirect to vendors dashboard
// ─────────────────────────────────────
router.get('/vendor/:id', requireLogin, (req, res) => {
  res.redirect('/vendors/dashboard/' + req.params.id);
});
