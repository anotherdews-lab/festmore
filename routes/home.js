// routes/home.js — MULTI-LANGUAGE VERSION with Events After Hero
...
router.get('/', (req, res) => {
  const featured      = db.prepare(`SELECT * FROM events WHERE status='active' AND featured=1 ORDER BY attendees DESC LIMIT 6`).all();
  const recent        = db.prepare(`SELECT * FROM events WHERE status='active' ORDER BY featured DESC, attendees DESC, id DESC LIMIT 6`).all();
  const topEvents     = recent.length >= 6 ? recent : db.prepare(`SELECT * FROM events WHERE status='active' ORDER BY attendees DESC LIMIT 6`).all();
  ...
  res.send(renderHome({ featured, topEvents, articles, countryCounts, catCounts, stats, user: req.session.user, tr, lang, langHtml: langSwitcher(req) }));
});
...
// ─── MAIN RENDER ───
function renderHome({ featured, topEvents, articles, countryCounts, catCounts, stats, user, tr, lang, langHtml }) {
  const ev = stats.events;
  const vn = stats.vendors;
  const ar = stats.articles;
  const sb = stats.subscribers;
  const isRtl = tr.dir === 'rtl';
  const showEvents = topEvents && topEvents.length > 0 ? topEvents : featured;
...
<meta name="description" content="Discover ${ev}+ festivals, markets and events worldwide. List your event free or find verified vendors for your festival. Covering Europe, India, Japan, Thailand and more."/>
...
<meta property="og:description" content="Discover ${ev}+ festivals, markets and events worldwide. List your event or find verified vendors."/>
...
<meta name="twitter:title" content="Festmore — Festivals, Markets &amp; Events Worldwide"/>
<meta name="twitter:description" content="Discover ${ev}+ festivals, markets and events worldwide."/>
...
<script type="application/ld+json">{"@context":"https://schema.org","@type":"Organization","name":"Festmore","url":"https://festmore.com","description":"Worldwide festival and vendor marketplace","contactPoint":{"@type":"ContactPoint","email":"hello@festmore.com","contactType":"customer service"}}</script>
...
${isRtl ? '<style>body{font-family:\'Bricolage Grotesque\',sans-serif;}.fm-hero-content,.fm-dual-cards,.fm-why-grid,.fm-how-steps,.fm-hero-stats{direction:rtl;}</style>' : ''}
<style>
...
/* EVENTS SECTION */
.fm-events-section{background:#fff;padding:64px 0 48px;}
.fm-events-inner{max-width:1440px;margin:0 auto;padding:0 40px;}
.fm-events-header{display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:32px;}
.fm-events-title{font-family:'DM Serif Display',serif;font-size:clamp(24px,3vw,40px);font-weight:400;color:#1a1612;margin-bottom:6px;}
.fm-events-sub{font-size:15px;color:#7a6f68;}
.fm-events-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:20px;}
.fm-event-card{background:#faf8f3;border:1px solid #e8e2d9;border-radius:20px;overflow:hidden;transition:all .25s;cursor:pointer;}
.fm-event-card:hover{border-color:#e8470a;box-shadow:0 20px 60px rgba(26,22,18,.12);transform:translateY(-4px);}
.fm-event-img{height:200px;position:relative;overflow:hidden;}
.fm-event-img img{width:100%;height:100%;object-fit:cover;transition:transform .4s;}
.fm-event-card:hover .fm-event-img img{transform:scale(1.06);}
.fm-event-overlay{position:absolute;inset:0;background:linear-gradient(to top,rgba(26,22,18,.6) 0%,transparent 60%);}
.fm-event-badges{position:absolute;top:12px;left:12px;display:flex;gap:6px;flex-wrap:wrap;}
.fm-event-badge{padding:3px 10px;border-radius:99px;font-size:11px;font-weight:700;}
.fm-event-price-tag{position:absolute;bottom:12px;right:12px;background:rgba(26,22,18,.8);color:#fff;padding:4px 12px;border-radius:99px;font-size:12px;font-weight:700;}
.fm-event-body{padding:18px 20px;}
.fm-event-meta{display:flex;align-items:center;gap:8px;margin-bottom:8px;}
.fm-event-date{font-size:12px;font-weight:700;color:#e8470a;text-transform:uppercase;letter-spacing:.6px;}
.fm-event-country{font-size:13px;}
.fm-event-title{font-family:'DM Serif Display',serif;font-size:19px;font-weight:400;color:#1a1612;margin-bottom:6px;line-height:1.25;}
.fm-event-city{font-size:13px;color:#7a6f68;margin-bottom:12px;}
.fm-event-footer{display:flex;justify-content:space-between;align-items:center;padding-top:12px;border-top:1px solid #e8e2d9;}
.fm-event-visitors{font-size:12px;color:#b5ada6;}
.fm-event-cta{font-size:13px;font-weight:700;color:#e8470a;}
.fm-events-cta-row{text-align:center;margin-top:36px;}

/* TRENDING BAR */
.fm-trending{background:#1a1612;padding:20px 0;}
.fm-trending-inner{max-width:1440px;margin:0 auto;padding:0 40px;display:flex;align-items:center;gap:16px;flex-wrap:wrap;justify-content:center;}
.fm-trending-label{font-size:12px;font-weight:700;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.8px;white-space:nowrap;}
.fm-trending-pill{background:rgba(255,255,255,.07);color:rgba(255,255,255,.7);padding:6px 16px;border-radius:99px;font-size:13px;font-weight:600;text-decoration:none;transition:all .2s;border:1px solid rgba(255,255,255,.1);white-space:nowrap;}
...
@media(max-width:900px){
  ...
  .fm-dual-inner,.fm-why-inner,.fm-proof-inner,.fm-how-inner,.fm-events-inner{padding:0 24px;}
  .fm-urgency-inner,.fm-trust-inner,.fm-trending-inner{padding:0 24px;gap:16px;}
}
...
<section class="fm-hero">
  ...
  <div class="fm-hero-content">
    ...
    <div class="fm-hero-right">
      <div class="fm-float-card fm-card-1"><span class="fm-card-icon">🎪</span><div class="fm-card-title">Pol'and'Rock Festival 2026</div><div class="fm-card-sub">750,000 visitors expected · Free entry</div><span class="fm-card-price">✅ Verified Listing</span></div>
      <div class="fm-float-card fm-card-2"><span class="fm-card-icon">🏪</span><div class="fm-card-title">Nordic Street Food Co.</div><div class="fm-card-sub">Copenhagen · Food & Drinks · 84 events</div><span class="fm-card-price fm-card-green">✅ Verified Vendor</span></div>
      <div class="fm-float-card fm-card-3"><span class="fm-card-icon">💰</span><div class="fm-card-title">New Application!</div><div class="fm-card-sub">Bratwurst Brothers applied to Glastonbury 2026</div></div>
    </div>
  </div>
</section>
...
<!-- ★ EVENTS YOU CANNOT MISS — RIGHT AFTER HERO ★ -->
<section class="fm-events-section">
  <div class="fm-events-inner">
    <div class="fm-events-header">
      <div>
        <div style="display:inline-flex;align-items:center;gap:6px;background:rgba(232,71,10,.08);border:1px solid rgba(232,71,10,.18);color:#e8470a;font-size:11px;font-weight:800;padding:3px 14px;border-radius:99px;margin-bottom:12px;letter-spacing:1px;text-transform:uppercase;">🔥 Must-See Events</div>
        <h2 class="fm-events-title">Events You Cannot Miss</h2>
        <p class="fm-events-sub">The biggest festivals, markets and concerts happening now</p>
      </div>
      <a href="/events" style="font-size:14px;font-weight:700;color:#e8470a;text-decoration:none;white-space:nowrap;">View all ${ev}+ events →</a>
    </div>

    <div class="fm-events-grid">
      ${showEvents.map(e => fmEventCard(e)).join('')}
    </div>

    <div class="fm-events-cta-row">
      <a href="/events" style="display:inline-flex;align-items:center;gap:10px;background:#e8470a;color:#fff;padding:16px 48px;border-radius:12px;font-size:16px;font-weight:700;text-decoration:none;box-shadow:0 8px 32px rgba(232,71,10,.35);transition:all .2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform=''">
        Explore All ${ev}+ Events →
      </a>
      <div style="margin-top:14px;font-size:13px;color:#b5ada6;">
        Festivals · Markets · Concerts · Christmas Markets · Free Events · and more
      </div>
    </div>
  </div>
</section>
...
<div class="fm-trending">
  <div class="fm-trending-inner">
    ...
    ${[
      ...
      ["Diwali India 2025", "/events?q=diwali"],
...
<section class="fm-why">
  <div class="fm-why-inner">
    ...
    <div class="fm-why-grid">
      ${[
        ['🌍','Worldwide Coverage','Your listing is visible to visitors, organisers and vendors across Europe, India, Japan, Thailand and beyond.'],
        ['🔍','SEO-Optimised Pages','Every listing gets its own page that ranks on Google. Visitors searching for events in your city find you directly.'],
  ...
  <div class="fm-dual-inner">
    ...
    <div class="fm-dual-cards">
      <div class="fm-plan-card fm-plan-organiser">
        ...
        ${['Live within 24 hours','Visible worldwide','SEO page with your own URL','Weekly newsletter inclusion','Connect with verified vendors','Track views and analytics'].map(b=>`<div class="fm-plan-benefit">✅ ${b}</div>`).join('')}
      ...
      <div class="fm-plan-card fm-plan-vendor">
        ...
        <span class="fm-plan-emoji">🏪</span>
        <h3 class="fm-plan-h">Get Booked at the World's Best Festivals and Markets</h3>
        ...
        ${['Verified badge on your profile','Apply directly to festivals and markets','Discovered by organisers worldwide','Featured in weekly newsletter','See who views your profile','Stand out from unverified competitors'].map(b=>`<div class="fm-plan-benefit">✅ ${b}</div>`).join('')}
...
function fmEventCard(e) {
  ...
  return `<article class="fm-event-card" onclick="window.location='/events/${e.slug}'" style="cursor:pointer;">
  <div class="fm-event-img">
    <img src="${img}" alt="${e.title}" loading="lazy"/>
    <div class="fm-event-overlay"></div>
    <div class="fm-event-badges">
      ${e.featured ? '<span class="fm-event-badge" style="background:#e8470a;color:#fff;">★ Featured</span>' : ''}
      <span class="fm-event-badge" style="background:rgba(0,0,0,.5);color:#fff;">${icon} ${e.category}</span>
    </div>
    <div class="fm-event-price-tag">${isFree ? '🟢 Free' : e.price_display || 'See website'}</div>
  </div>
  <div class="fm-event-body">
    <div class="fm-event-meta">
      <span class="fm-event-date">${e.date_display || e.start_date || ''}</span>
      <span class="fm-event-country">${flag}</span>
    </div>
    <h3 class="fm-event-title">${e.title}</h3>
    <div class="fm-event-city">📍 ${e.city}${e.country ? ', ' + (COUNTRY_NAMES[e.country] || e.country) : ''}</div>
    <div class="fm-event-footer">
      <span class="fm-event-visitors">${e.attendees ? (e.attendees).toLocaleString() + ' visitors' : ''}</span>
      <span class="fm-event-cta">View details →</span>
    </div>
  </div>
</article>`;
}

// ─── LEGACY EVENT CARD (used elsewhere) ───
